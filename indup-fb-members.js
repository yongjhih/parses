...
program
  ...
  .option('--from', 'Specifiy from group')
  .option('--to', 'Specifiy to groups')
...
if (!program.to) {
  console.error('missing to group');
  process.exit();
}
if (!program.from) {
  console.error('missing from groups');
  process.exit();
}
...
query.containedIn("groups", program.from.split(","));
Parses.all(query)
  .filter(function (it) {
    var groups = it.get("groups");
    return !!!~groups.indexOf(program.to);
  })
